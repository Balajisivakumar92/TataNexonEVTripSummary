...
if st.button("Generate Summary"):
    ...
    st.markdown(f"🛡️ **Safety Score**          : {safety_score if safety_score is not None else 'N/A'}")
    st.markdown(f"💥 **Harsh Braking Events**  : {harsh_braking if harsh_braking is not None else 'N/A'}")
    ...
    if safety_score is not None:
        st.markdown(f"- Safety Rating: {interpret_safety(safety_score)}")
        if safety_score < 7:
            st.warning("- ⚠️ Try maintaining more consistent speeds.")
    if harsh_braking is not None:
        if harsh_braking > 3:
